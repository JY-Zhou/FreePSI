...
void EMAlgorithm::correctInfeasibleX(int g) {
    X[g] /= X[g].sum();
    //X[g] = X[g].cwiseProduct(L[g].transpose().cwiseInverse());
    ...
    //Eigen::SparseMatrix<double> tempTheta = X[g].cwiseProduct(L[g].transpose()); 
    //tempTheta /= tempTheta.sum();
    //X[g] = tempTheta.cwiseProduct(L[g].transpose().cwiseInverse());
    X[g] /= X[g].sum();
    X[g].prune(EPS, 0.01);
    X[g].makeCompressed();
}
...
void EMAlgorithm::correctInfeasibleVariables() {
    ...
    //        preLikelihood += optimizer.QFunction(X[g]);
    //        //double tmp = optimizer.QFunction(X[g]);
    //        //cout << " Xnorm = " << X[g].norm() << " Qfunc = " << tmp << endl;
    //        //getchar();
    //    }
    //}
...
void EMAlgorithm::eStep() {
    ...
    clock_t st = clock();
    //Eigen::SparseMatrix<double> tot(NW, 1);
...
//    }
    ...
    for(int g = 0; g < NG; g ++) {
        ...
        if(isAvaliableZ[g] == 1) {
            clock_t st3 = clock();
            //Mu[g] = Mu[g].cwiseProduct(tot.cwiseInverse());
...
#pragma omp critical
#endif
        {
            ...
            //cout << "Optimizing gene" << g << " NE = " << NE[g] << " NX = " << NX[g] << endl;
        }
        ...
        if(isAvaliableZ[g] == 1) {
            ...
            if(X[g].nonZeros() > 0) {
                ConjugateGradientProjection optimizer(*this, g);
                //newX = Eigen::SparseMatrix<double>(optimizer.optimizeQ(eps, iter));
    ...
    //cout << "M-step time profiling" << endl; 
    //cout << "Average QFunc: " << ConjugateGradientProjection::QFunc_t / ConjugateGradientProjection::QFunc_n;
    //cout << " (call " << ConjugateGradientProjection::QFunc_n << " times)." << endl;
    //cout << "Average QGrad: " << ConjugateGradientProjection::QGrad_t / ConjugateGradientProjection::QGrad_n;
    //cout << " (call " << ConjugateGradientProjection::QGrad_n << " times)." << endl;
    //cout << "Average ConsAdd: " << ConjugateGradientProjection::ConsAdd_t / ConjugateGradientProjection::ConsAdd_n;
    //cout << " (call " << ConjugateGradientProjection::ConsAdd_n << " times)." << endl;
    //cout << "Total time usage = " << ConjugateGradientProjection::timer[0] << endl;
    //for(int i = 1; i < ConjugateGradientProjection::timer.size(); i ++) {
    //    if(i == 1) cout << "CGPA checker:"<<endl;
    //    if(i == 10) cout << "Add constraint checker: " << endl;
    //    if(i == 18) cout << "Gradient checker: " << endl;
    //    cout << "Part " << i << ": \t" << ConjugateGradientProjection::timer[i] << endl;
    //    cout << "\t\t" << ConjugateGradientProjection::timer[i] / ConjugateGradientProjection::timer[0] << endl;
    //}
    //cout << "Totally iteration time = " << ConjugateGradientProjection::iteration << endl;
    //cout << "Exit ports" << endl;
    //for(int i = 0; i < ConjugateGradientProjection::exitPort.size(); i ++) {
    //    cout << "Exit port " << i << "\t" << ConjugateGradientProjection::exitPort[i] << endl;
    //}
    //
    cout << " OK!" << endl;
}
...
//    for(int g = 0; g < NG; g ++) {
//        if(Z.coeffRef(g, 0) > EPS) {
//            Eigen::SparseMatrix<double> ZCXg = Z.coeffRef(g, 0) * C[g] * X[g];
//            ZCXg.prune(EPS, 0.01);
//            ZCXg.makeCompressed();
//            ZCX += ZCXg;
//        }
//    }
    ...
    //vector <Eigen::SparseMatrix <double> > ZCXg = vector <Eigen::SparseMatrix <double> > (NG);
    //for(int g = 0; g < NG; g ++) {
    //    ZCXg[g] = Eigen::SparseMatrix <double> (NW, 1);
    //}
    //cout <<"touch here?" << endl;
//#ifdef EIGEN_DONT_PARALLELIZE
//#pragma omp parallel for num_threads(nThread)
//#endif
    //for(int g = 0; g < NG; g ++) {
    //    if(isAvaliableZ[g] == 1) {
    //        ZCXg[g] = Z.coeffRef(g, 0) * C[g] * X[g];
    //    }
    //}
    //cout <<"touch here?" << endl;
...
#pragma omp parallel for num_threads(nThread) reduction(+:ZCX)
    ...
    for(int g = 0; g < NG; g ++) {
        if(isAvaliableZ[g] == 1) {
            ...
            //for(Eigen::SparseMatrix<double>::InnerIterator it(X[g], 0); it; ++ it) {
            //    ZCX += C[g].col(it.row()) * it.value() * Z.coeffRef(g, 0);
            //}
        }
    }
    //cout <<"touch here?" << endl;
    ...
    clock_t st3 = clock();
    //Eigen::MatrixXd ans = W.transpose() * temp;

    //int ans = W.col(0).dot(logZCX.col(0));
...
void EMAlgorithm::computeTrueLogLikelihood() {
    cout << "*** Start to compute groundtruth's likelihood ... " << endl;
    ifstream infile("./trueVars.out", ios::in);
    Eigen::SparseMatrix<double> trueZ(NG, 1);
    vector<Eigen::SparseMatrix<double> > trueX;
    for(int g = 0; g < NG; g ++) {
        trueX.push_back(Eigen::SparseMatrix<double>(NX[g], 1));
    }

    for(int g = 0; g < NG; g ++) {
        double tempZ;
        infile >> tempZ;
        trueZ.coeffRef(g, 0) = tempZ;
        vector <double> unordered_X;
        for(int s = 0; s < NX[g]; s ++) {
            double tempX;
            infile >> tempX;
            unordered_X.push_back(tempX);
        }
        
        vector <double> ordered_X(NX[g]);
        if(kmerHasher.geneStrand[g] == "-") {
            map <pair<int, int>, double> table;
            int i = 0;
            for(int e = 0; e < NE[g]; e ++) {
                table[make_pair(e, e)] = unordered_X[i++];
            }
            for(int ei = 0; ei < NE[g]; ei ++) {
                for(int ej = ei + 1; ej < NE[g]; ej ++) {
                    table[make_pair(ei, ej)] = unordered_X[i++];
                }
            }
            int s = 0;
            for(int e = 0; e < NE[g]; e ++) {
                ordered_X[s ++] = table[make_pair(NE[g] - e - 1, NE[g] - e - 1)];
            }
            for(int ei = 0; ei < NE[g]; ei ++) {
                for(int ej = ei + 1; ej < NE[g]; ej ++) {
                    ordered_X[s ++] = table[make_pair(NE[g] - ej - 1, NE[g] - ei - 1)];
                }
            }
        } else {
            for(int s = 0; s < NX[g]; s ++) {
                ordered_X[s] = unordered_X[s];
            }
        }

        for(int s = 0; s < NX[g]; s ++) {
            trueX[g].coeffRef(s, 0) = ordered_X[s];
        }

        if(tempZ > EPS) {
            trueX[g] /= trueX[g].sum();
            Eigen::SparseMatrix<double> ieq = A[g] * trueX[g];
            for(int i = 0; i < ieq.rows(); i ++) {
                if(ieq.coeffRef(i, 0) < - EPS) {
                    cout << "Error: Still infeasible point! (Violate Theta inequalities)" << endl;
                    cout << ieq.coeffRef(i, 0) << " in gene " << g << " constraint " << i << endl;
                    //exit(-1);
                }
            }
            Eigen::MatrixXd eq = Eigen::MatrixXd::Ones(1, NX[g]) * trueX[g];
            if(fabs(eq(0, 0) - 1) > EPS) {
                cout << "Error: Still infeasible point! (Violate Theta equalities)" << endl;
                cout << eq(0, 0) << " in gene " << g << endl;
                //exit(-1);
            }
        }
    }
    if(fabs(trueZ.sum() - 1.00000) > EPS * 100.0) {
        cout << "Error: Still infeasible point! (Violate Gamma equalities)" << endl;
        cout << fixed << setprecision(9) << "Sum Gamma = " << trueZ.sum() << endl;
        //exit(-1);
    }
    ...
    temp.resize(NW, 1);
    for(int g = 0; g < NG; g ++) {
        temp += trueZ.coeffRef(g, 0) * (C[g] * trueX[g]);
    }
    temp.prune(EPS, 0.01);
    temp.makeCompressed();
    int nz = temp.nonZeros();
    ...
    Eigen::MatrixXd ans = W.transpose() * temp;
    cout << "=== The log-likelihood of ground truth is " << ans(0, 0) << endl;
    cout << "*** Finish computing groundtruth's likelihood ... " << endl;
}
...
void EMAlgorithm::work(string chrName) {
    ...
    for(int it = 1; it <= maxIter; it ++) {
        ...
        //if((newLikelihood - preLikelihood) < 1) {
        //if(it > 40) {
            for(int g = 0; g < NG; g ++) {
...
void EMAlgorithm::computePSI(string expName) {
    ...
    for(int g = 0; g < NG; g ++) {
        ...
        if(isAvaliableZ[g] == 1) {
            //Eigen::SparseMatrix<double> tempPSI = X[g];
            //cout << X[g] << endl;
    ...
    //ofstream zlLog("./zlLog", ios::out);
    //for(int g = 0; g < NG; g ++) {
    //    zlLog << Z.coeffRef(g, 0) << "\t" << L[g].sum() << endl;
    //}

    string tpmFile = expName + "/tpm_freePSI_raw.json";
    ofstream tpmOutput(tpmFile, ios::out);
    vector <double> tpm = vector <double> (NG);
    double sumTPM = 0.0;
    for(int g = 0; g < NG; g ++) {
        tpm[g] = Z.coeffRef(g, 0) / L[g].sum();
        sumTPM += tpm[g];
    }
    for(int g = 0; g < NG; g ++) {
        tpm[g] /= sumTPM;
